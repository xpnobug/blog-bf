...
// console["log"]("%c 获取图片api %c", "color: #fadfa3; background: #030307; padding:5px", "background: #fadfa3; padding:5px");
...
function addJigsaw(img, alt) {
    ...
    if (jigsaw.halfHtml !== '') {
        ...
        newHtml = '<div class="Hhalf oneImg"><a href="#" data-fancybox="images"><img  src="' + decode360Url(img, imgWidth, imgHeight, 0) + '" alt="' + alt + '"title="关键字：' + alt + '" class="pimg"  onclick="changeBg(\'url('+decode360Url(img, imgWidth, imgHeight, 0)+')\')" ></a>    </div></div>';
    ...
    if ((jigsaw.loadBig === false) && ((Math.floor(Math.random() * 3) === 0) || ((jigsaw.count % 5) === 0))) {
        ...
        imgHeight = parseInt(imgWidth * seting.ratio);
        newHtml = '<div class="item half oneImg"><a href="#" data-fancybox="images"><img src="' + decode360Url(img, imgWidth, imgHeight, 0) + '" alt="' + alt + '" title="关键字：' + alt + '" class="pimg" onclick="changeBg(\'url('+decode360Url(img, imgWidth, imgHeight, 0)+')\')" ></a></div>';
        contAdd(newHtml);
        jigsaw.loadBig = true;
        return true
    }
    imgWidth = parseInt(screen.width / 4);
    imgHeight = parseInt(imgWidth * seting.ratio);
    jigsaw.halfHtml = '<div class="item quater">    <div class="Hhalf oneImg"><a href="#" data-fancybox="images"><img src="' + decode360Url(img, imgWidth, imgHeight, 0) + '" alt="' + alt + '" title="关键字：' + alt + '" class="pimg" onclick="changeBg(\'url('+decode360Url(img, imgWidth, imgHeight, 0)+')\')" ></a>    </div>';
    return true
}
